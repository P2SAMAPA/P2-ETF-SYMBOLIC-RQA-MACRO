...
st.set_page_config(page_title="Symbolic RQA with Macro‑Conditioned Thresholds", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🔁 Symbolic RQA with Macro‑Conditioned Thresholds</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Recurrence quantification | Threshold adapts to macro state</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 RQA")
...
st.sidebar.markdown(f"**Embedding dim E:** {config.EMBEDDING_DIM} | **τ:** {config.TIME_DELAY}")
st.sidebar.markdown(f"**Base quantile:** {config.BASE_QUANTILE} | **γ:** {config.GAMMA}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'rqa_macro_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>RQA score: {etf['rqa_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized RQA"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized RQA", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🔁 Top ETFs by Macro‑Conditioned RQA Recurrence Rate (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Recurrence quantification analysis** (RQA) measures how often a system revisits similar states.
        - We reconstruct state space using time‑delayed embedding of ETF returns.
        - The recurrence threshold is **dynamically adjusted** using all macro variables (VIX, DXY, yields) – a composite factor from ridge regression.
        - **Recurrence rate** = proportion of points close to the last state in the state space.
        - High recurrence rate → dynamics are highly structured and predictable given current macro conditions.
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the macro‑conditioned RQA scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Symbolic RQA | Recurrence with macro‑conditioned thresholds")
