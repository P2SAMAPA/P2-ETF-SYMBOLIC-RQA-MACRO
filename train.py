...
from symbolic_rqa import rqa_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    top_etfs = [{"ticker": t, "rqa_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/rqa_macro_{timestamp}.json"
